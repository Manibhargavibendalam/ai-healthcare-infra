# FAILURE 7 (recover): remove the bad override, recreate the worker, verify drain.
param([string]$A = 'http://127.0.0.1:8080', [int]$TimeoutSec = 180)
$ErrorActionPreference = 'Stop'
Push-Location (Split-Path $PSScriptRoot -Parent)
try {
  Remove-Item -LiteralPath 'compose.configbad.yaml' -ErrorAction SilentlyContinue
  ...
  Write-Output "RECOVERED. queue depth: $depth (expect 0)"
